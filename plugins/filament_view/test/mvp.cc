...
void addQuadToScene() {
  ...
  indexBuffer->setBuffer(
    *gContext.engine,
    IndexBuffer::BufferDescriptor(indices, indexBuffer->getIndexCount() * sizeof(uint32_t))
  );
  ...
  gContext.quad.material =
    Material::Builder().package(package.getData(), package.getSize()).build(*gContext.engine);
  gContext.quad.material->setDefaultParameter("baseColor", RgbType::LINEAR, float3{0, 1, 0});
...
void setDefaultCamera() {
  ...
  gContext.camera->setProjection(
    60, static_cast<float>(viewport.width) / static_cast<float>(viewport.height), 0.1, 10
  );
...
int main() {
  ...
  gContext.swapChain = gContext.engine->createSwapChain(getNativeWindow(window));
  ...
  /// Skybox
  gContext.skybox = Skybox::Builder().color({0.5, 0.5, 0.5, 1.0}).build(*gContext.engine);
  ...
  while (keep_window_open) {
    ...
    while (SDL_PollEvent(&e) > 0) {
      ...
      if (prev_width != gWidth || prev_height != gHeight) {
        ...
        gContext.camera->setProjection(
          60, static_cast<float>(gWidth) / static_cast<float>(gHeight), 0.1, 10
        );
      }
