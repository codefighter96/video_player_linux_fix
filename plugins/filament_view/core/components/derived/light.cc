...
Light::Light(const flutter::EncodableMap& params)
  ...
  if (const auto it = params.find(flutter::EncodableValue(kType)); it != params.end()) {
    if (const auto& snd = it->second; std::holds_alternative<std::string>(snd)) {
  ...
  Deserialize::DecodeParameterWithDefault(
    kDirection, &m_f3Direction, params, filament::math::float3(0, -1, 0)
  );
  Deserialize::DecodeParameterWithDefault(kCastLight, &m_bCastLight, params, true);
  Deserialize::DecodeParameterWithDefault(kCastShadows, &m_bCastShadows, params, true);
  Deserialize::DecodeParameterWithDefault(kFalloffRadius, &m_fFalloffRadius, params, 0.0f);
  Deserialize::DecodeParameterWithDefault(
    kSpotLightConeInner, &m_fSpotLightConeInner, params, 0.0f
  );
  Deserialize::DecodeParameterWithDefault(
    kSpotLightConeOuter, &m_fSpotLightConeOuter, params, 0.0f
  );
  Deserialize::DecodeParameterWithDefault(kSunAngularRadius, &m_fSunAngularRadius, params, 0.0f);
  Deserialize::DecodeParameterWithDefault(kSunHaloSize, &m_fSunHaloSize, params, 0.0f);
  Deserialize::DecodeParameterWithDefault(kSunHaloFalloff, &m_fSunHaloFalloff, params, 0.0f);
}
...
void Light::DebugPrint(const std::string& tabPrefix) const {
  ...
  spdlog::debug(
    tabPrefix + "Position: x={}, y={}, z={}", m_f3Position.x, m_f3Position.y, m_f3Position.z
  );
  spdlog::debug(
    tabPrefix + "Direction: x={}, y={}, z={}", m_f3Direction.x, m_f3Direction.y, m_f3Direction.z
  );
...
filament::LightManager::Type Light::textToLightType(const std::string& type) {
  static constexpr std::pair<const char*, filament::LightManager::Type> typeMap[] = {
...
const char* Light::lightTypeToText(const filament::LightManager::Type type) {
  static constexpr std::pair<filament::LightManager::Type, const char*> typeMap[] = {
