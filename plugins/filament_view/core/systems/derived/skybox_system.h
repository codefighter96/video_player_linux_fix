...
class SkyboxSystem : public ECSystem {
  public:
    ...
    static std::future<Resource<std::string_view>> setSkyboxFromKTXAsset(const std::string& path);

    static std::future<Resource<std::string_view>> setSkyboxFromKTXUrl(const std::string& url);

    static std::future<Resource<std::string_view>> setSkyboxFromColor(const std::string& color);
    ...
    [[nodiscard]] static size_t StaticGetTypeID() { return typeid(SkyboxSystem).hash_code(); }
