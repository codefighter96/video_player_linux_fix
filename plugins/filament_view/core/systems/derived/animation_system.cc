...
void AnimationSystem::vInitSystem() {
  ...
  vRegisterMessageHandler(ECSMessageType::AnimationEnqueue, [this](const ECSMessage& msg) {
    ...
    const EntityGUID& guid = msg.getData<EntityGUID>(ECSMessageType::EntityToTarget);
    ...
    if (const auto it = _entities.find(guid); it != _entities.end()) {
      const auto animationComponent = dynamic_cast<Animation*>(
        it->second->GetComponentByStaticTypeID(Animation::StaticGetTypeID()).get()
      );
...
void AnimationSystem::vUpdate(const float fElapsedTime) {
  for (auto& [fst, snd] : _entities) {
    const auto animator =
      dynamic_cast<Animation*>(snd->GetComponentByStaticTypeID(Animation::StaticGetTypeID()).get());
    animator->vUpdate(fElapsedTime);
  }
}

////////////////////////////////////////////////////////////////////////////////////
void AnimationSystem::vRegisterEntityObject(const std::shared_ptr<EntityObject>& entity) {
...
////////////////////////////////////////////////////////////////////////////////////
void AnimationSystem::vUnregisterEntityObject(const std::shared_ptr<EntityObject>& entity) {
...
void AnimationSystem::vNotifyOfAnimationEvent(
  ...
  const auto event = flutter::EncodableMap(
    {{flutter::EncodableValue("event"), flutter::EncodableValue(kAnimationEvent)},
     ...
     {flutter::EncodableValue(kGlobalGuid), flutter::EncodableValue(entityGuid)},
