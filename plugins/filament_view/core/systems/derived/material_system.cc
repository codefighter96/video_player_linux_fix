...
Resource<filament::MaterialInstance*> MaterialSystem::getMaterialInstance(
  ...
  if (!materialDefinitions) {
    SPDLOG_ERROR("--Bad MaterialDefinitions Result "
  ...
  if (const auto materialToInstanceFromIter = loadedTemplateMaterials_.find(lookupName);
    ...
    if (materialToInstanceFrom.getStatus() != Status::Success) {
      spdlog::error("--Bad Material Result MaterialManager::getMaterialInstance");
...
void MaterialSystem::vInitSystem() {
  vRegisterMessageHandler(ECSMessageType::ChangeMaterialParameter, [this](const ECSMessage& msg) {
    ...
    const auto objectLocatorSystem =
      ECSystemManager::GetInstance()->poGetSystemAs<EntityObjectLocatorSystem>(
        EntityObjectLocatorSystem::StaticGetTypeID(), "ChangeMaterialParameter"
      );

    if (const auto entityObject = objectLocatorSystem->poGetEntityObjectById(guid);
  ...
  vRegisterMessageHandler(ECSMessageType::ChangeMaterialDefinitions, [this](const ECSMessage& msg) {
    ...
    const auto objectLocatorSystem =
      ECSystemManager::GetInstance()->poGetSystemAs<EntityObjectLocatorSystem>(
        EntityObjectLocatorSystem::StaticGetTypeID(), "ChangeMaterialDefinitions"
      );
