...
class ECSystemManager {
  public:
    ...
    void vRemoveSystem(const std::shared_ptr<ECSystem>& system) {
      ...
      m_vecSystems.erase(
        std::remove(m_vecSystems.begin(), m_vecSystems.end(), system), m_vecSystems.end()
      );
    }
    ...
    [[nodiscard]] const std::thread& GetFilamentAPIThread() const { return filament_api_thread_; }

    [[nodiscard]] std::unique_ptr<asio::io_context::strand>& GetStrand() { return strand_; }

    template <typename T> void setConfigValue(const std::string& key, T value) {
