...
class ECSMessage {
  public:
    ...
    template <typename T> void addData(ECSMessageType type, const T& value) { data_[type] = value; }

    // Get data from the message
    template <typename T> T getData(ECSMessageType type) const {
      ...
      if (it != data_.end()) {
        try {
          ...
          throw std::runtime_error(
            "Type mismatch for key. Expected type: " + std::string(typeid(T).name())
          );
        }
      }
