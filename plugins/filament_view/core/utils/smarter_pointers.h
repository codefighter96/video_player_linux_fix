...
// — std::weak_ptr<T> —
template <typename T>
struct pointer_access<std::weak_ptr<T>> {
  using element_type = T;
  // lock() -> shared_ptr -> get()
...
struct pointer_access<
    P,
    std::void_t<decltype(std::declval<P>().get()),
                decltype(std::declval<P>().reset(
                    std::declval<typename P::element_type*>()))>> {
...
template <typename T>
using smarter_shared_ptr = smarter_ptr<T, std::shared_ptr<T>>;
template <typename T>
using smarter_unique_ptr = smarter_ptr<T, std::unique_ptr<T>>;
template <typename T>
using smarter_weak_ptr = smarter_ptr<T, std::weak_ptr<T>>;
template <typename T>
using smarter_raw_ptr = smarter_ptr<T, T*>;
