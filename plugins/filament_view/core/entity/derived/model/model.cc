...
////////////////////////////////////////////////////////////////////////////
Model::Model(std::string assetPath, std::string url, const flutter::EncodableMap& params)
  ...
  Deserialize::DecodeParameterWithDefault(
    kRenderable_KeepAssetInMemory, &m_bShouldKeepAssetDataInMemory, params, false
  );

  Deserialize::DecodeParameterWithDefault(
    kRenderable_IsPrimaryAssetToInstanceFrom, &m_bIsPrimaryAssetToInstanceFrom, params, false
  );
...
void Model::vInitComponents(
  ...
  if (const auto it = params.find(flutter::EncodableValue(kAnimation));
      it != params.end() && !it->second.IsNull()) {
    auto animationInformation =
      std::make_shared<Animation>(std::get<flutter::EncodableMap>(it->second));
    vAddComponent(std::move(animationInformation));
  }
}

////////////////////////////////////////////////////////////////////////////
GlbModel::GlbModel(std::string assetPath, std::string url, const flutter::EncodableMap& params)
...
Model::Deserialize(const std::string& /*flutterAssetsPath*/, const flutter::EncodableMap& params) {
  ...
  for (const auto& [fst, snd] : params) {
    ...
    } else if (key == "pathPrefix" && std::holds_alternative<std::string>(snd)) {
      pathPrefix = std::get<std::string>(snd);
    } else if (key == "pathPostfix" && std::holds_alternative<std::string>(snd)) {
      pathPostfix = std::get<std::string>(snd);
    } else if (key == "scene" && std::holds_alternative<flutter::EncodableMap>(snd)) {
  ...
  if (is_glb) {
    ...
    toReturn->vInitComponents(std::move(oTransform), std::move(oCommonRenderable), params);
    return toReturn;
  }
...
void Model::vChangeMaterialDefinitions(
  ...
  if (getAsset()) {
    ...
    for (size_t i = 0; i < instanceEntityCount; i++) {
      ...
      if (const utils::Entity entity = instanceEntities[i]; renderManager.hasComponent(entity)) {
        const auto ri = renderManager.getInstance(entity);
