...
class EntityObject : public std::enable_shared_from_this<EntityObject> {
  ...
  public:
    ...
    bool operator==(const EntityObject& other) const { return global_guid_ == other.global_guid_; }
    ...
    [[nodiscard]] bool HasComponentByStaticTypeID(size_t staticTypeID) const {
      return std::any_of(components_.begin(), components_.end(), [staticTypeID](const auto& item) {
        return item->GetTypeID() == staticTypeID;
      });
    }

    [[nodiscard]] const std::string& GetGlobalGuid() const { return global_guid_; }
  ...
  protected:
    ...
    void vRemoveComponent(size_t staticTypeID) {
      components_.erase(
        std::remove_if(
          components_.begin(),
          components_.end(),
          [&](auto& item) { return item->GetTypeID() == staticTypeID; }
        ),
        components_.end()
      );
    }
    ...
    [[nodiscard]] std::shared_ptr<Component> GetComponentByStaticTypeID(size_t staticTypeID) {
    ...
    [[nodiscard]] std::shared_ptr<Component> GetComponentByStaticTypeID(size_t staticTypeID) const {
      for (const auto& item : components_) {
        if (item->GetTypeID() == staticTypeID) {
          return item;
        }
